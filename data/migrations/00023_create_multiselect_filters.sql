...
CREATE
OR REPLACE FUNCTION check_filter_type() RETURNS TRIGGER AS
$$
BEGIN
IF NOT EXISTS (
    SELECT
...
INSERT
    OR
UPDATE
    ON multiselect_values FOR EACH ROW EXECUTE FUNCTION check_filter_type();

-- Function to update parent filter timestamp
CREATE
OR REPLACE FUNCTION update_parent_filter_timestamp() RETURNS TRIGGER AS
$$
BEGIN
UPDATE
    filters
