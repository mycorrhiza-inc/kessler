...
func unrollSingleSchema(schema dbstore.SemiCompleteFileListGetRow) (files.CompleteFileSchema, error) {
	...
	if err != nil {
		errorstring := fmt.Sprintf("Error Unmarshalling file {%v} metadata : %v\n", schema.ID, err)
...
func doShittyHydrationAAA(uuidList []uuid.UUID, ctx context.Context, q dbstore.Queries) ([]files.CompleteFileSchema, error) {
	fmt.Printf("Got Hydration Request for %v files", uuidList)
